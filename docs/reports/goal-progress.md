...
- **Verification:** focused Mac onboarding/window tests passed (`3` tests after the chrome change; final onboarding suite `2` tests), `./scripts/check.sh` passed, changed-file `git diff --check` passed, and the verify package built and installed at `/Applications/Bessie Verify.app`. Packaged/installed executable SHA-256 is `084b4dc884525f06b11849bf2558c94ab743bc2c280814a0d8362244135de498`. An isolated fresh-profile screenshot at `/tmp/bessie-final-cow.png` confirms cow-only top-left chrome and two compact equal-height host cards without clipping. The production app was not replaced because its separate release metadata/signing gate remains unfinished.
