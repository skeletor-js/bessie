...
## 2026-08-09: Complete dirty-tree checkpoint and signed production install

- Corrected the final inventory of the macOS motion-quality work: U1-U5 were already implemented and accepted, not merely planned. The current source retains the shared 160/200 ms motion contract, linear working indicator, instant command palette, localized hierarchy fades, onboarding continuity, Projects transitions, and explicit Reduce Motion branches.
- Committed the complete previously dirty tree as `09bcc1c` (`feat: complete V1 product polish and release groundwork`), covering motion quality, onboarding, raw Done/Idle presentation, terminal/pane interaction work, secure Sparkle packaging and update coordination, the staged appcast site workflow, and reconciled future iOS documents.
- **Verification:** `./scripts/check.sh` passed. A full Mac `xcrun swift test --disable-keychain --disable-netrc` run passed **677 XCTest cases with 6 skipped and 0 failures**, followed by **21 Swift Testing tests in 3 suites with 0 failures**. The secret scan found only deliberate private-key rejection fixtures and ignored dependency examples. The complete diff's only whitespace findings were the existing intentional Markdown hard breaks recorded in the iOS plans.
- **Production install identity:** packaged and installed `/Applications/Bessie.app` executables are byte-identical at SHA-256 `96a85e5b9d4fe9264b746ddf7df899423e713010a6c49268517cf9234e0ab860`. The installed bundle is `dev.bessie.app`, version `0.1.0` build `9`, has a valid strict deep Developer ID signature for Team ID `T4K7A3GPQ6`, and relaunched from the installed path as PID `79972`. A GUI-session screenshot at `/tmp/bessie-build9-installed.png` shows live Herdr content, cow-only top-left in-window chrome, and no obvious clipping or rendering regression. Gatekeeper assessment remains rejected solely because this local dogfood build is Developer ID-signed but unnotarized; notarization remains required before distribution.

## 2026-08-09: Onboarding host selection and cow-only chrome

- Setup now treats host choice as setup-session state: **This Mac** and **Remote over SSH** are always explicit choices, a disabled local connection can be re-enabled without replacing the Project default, fresh setup does not preselect a retained production host, local folders use `NSOpenPanel`, and remote setup is available inline.
- Removed the visible `Bessie` wordmark from the onboarding rail, main Herd rail, Trouble/Connect product mark, and native main-window title treatment. The cow remains visible and retains the semantic accessibility label. The centered cold-open splash name and the macOS application/menu identity are intentionally unchanged.
- Increased host-card horizontal breathing room and added a local subtitle. Both host cards are constrained to the same 76-point height; the remote menu label does not add a second layer of vertical padding.
- **Verification:** focused Mac onboarding/window tests passed (`3` tests after the chrome change; final onboarding suite `2` tests), `./scripts/check.sh` passed, changed-file `git diff --check` passed, and the verify package built and installed at `/Applications/Bessie Verify.app`. Packaged/installed verify executable SHA-256 is `084b4dc884525f06b11849bf2558c94ab743bc2c280814a0d8362244135de498`. An isolated fresh-profile screenshot at `/tmp/bessie-final-cow.png` confirms cow-only top-left chrome and two compact equal-height host cards without clipping. The later signed production install is recorded in the checkpoint above.

## 2026-08-09: Raw Done/Idle status presentation and conditional Unknown

- Removed the synthetic user-facing Settled filter, presentation status, rail group, menu total, onboarding legend row, card label, command-palette search term, and accessibility vocabulary. Herdr `done` and `idle` now remain distinct as **Done** and **Idle** through filtering, counting, sorting, cards, rail projection, palette rows/search, pane speech, onboarding, and menu-bar totals.
- Added non-color-only status geometry: Done is a checkmark ring and Idle is a horizontal line. Their spoken labels are exactly `Done, checkmark ring` and `Idle, horizontal line`. Needs-you/blocked and Working behavior are unchanged.
- Unknown remains a live raw model/count. Herd filter controls use the scoped count, rail sections and menu totals hide at zero, and menu-bar badge accessibility omits the zero-Unknown phrase. A selected Unknown Herd filter derives All immediately and writes the normalized selection when its scoped count drops to zero; it reappears live above zero.
- Replaced fixed `prefix(4)` rail traversal with the explicit five-state `HerdRailGroup.statusCases`, preserving every Done, Idle, and Unknown row after the split. Pinned/snoozed precedence, attention contribution, stored pane incarnations, and snooze/pin persistence are unchanged.
- Kept the persisted notification case named `blockedAndDone` and preserved its existing blocked plus active→done/idle trigger and idle↔done deduplication behavior. User-facing idle notifications now say `is idle`; the Settings label is `Needs me, Done, and Idle`.
- Updated the governing Mac status, palette, tab-refresh, pinned/snoozed, roadmap, and future iOS status documents. Two conflicting historical goal-progress claims are explicitly marked as superseded; unrelated ordinary English and `session-settled:` metadata remain untouched.
- **RED contract:** focused tests were changed before production code for distinct filters/counts/search/order, fifth-state traversal, Unknown visibility/normalization, separate menu counts, zero-Unknown accessibility, glyph geometry/spoken labels, and Idle notification copy. The fresh Mac RED scratch `/tmp/bessie-status-red.6hrMho` reached Swift source compilation, but `jordan-macbook` went offline before XCTest returned; no RED result is claimed.
- **Linux verification:** the focused status-contract static audit passed. Status-scoped `git diff --check` passed for source, tests, scripts, Mac governing docs, and this report. The static status and UI-copy guards now require distinct Done/Idle and five-state onboarding copy. `./scripts/check.sh` reached and passed Sparkle packaging, release tooling, Herdr runtime/packaging, intent parity, native theme-color, Catppuccin contract, and region-evidence checks, then exited 1 in `check-ui-copy.sh` because the pre-existing dirty mouse work contains the comment `local/Herdr-owned` in `TerminalPaneController.swift:2075`; that file was not changed for this status work. The complete diff check remains blocked by pre-existing Markdown hard-break whitespace in the already-dirty iOS plan/roadmap. Swift is unavailable on the VPS.
- **Native verification queued, not run:** once the Mac returns, first run the preserved RED scratch command below, then sync the final tree into a new scratch and run the GREEN focused suite plus release build. Native tests, release compilation, and screenshots remain **unverified due to the offline Mac**. No package, install, relaunch, `mac-verify.sh`, commit, push, PR, publication, deployment, credential use, or Herdr-owned runtime mutation occurred.

```bash
# Observe the preserved test-first RED tree, if /tmp survived the outage.
ssh jordan-macbook "cd /tmp/bessie-status-red.6hrMho && xcrun swift test --disable-keychain --disable-netrc --filter 'HerdListTests|CommandPaletteIndexTests|BessieVisualFoundationTests|HerdRailPresentationTests|MenuBarPresentationTests|SurfaceProjectionTests/testNotificationPlannerEmitsInitialNothingThenTransitionsAndRoutes'"

# GREEN in a new scratch, executed from /home/hermes/code/bessie on the VPS.
scratch=$(ssh jordan-macbook 'mktemp -d /tmp/bessie-status-green.XXXXXX')
rsync -a --exclude='.git' --exclude='.build' --exclude='.local' --exclude='.worktrees' --exclude='dist' --exclude='site/node_modules' ./ "jordan-macbook:$scratch/"
ssh jordan-macbook "cd '$scratch' && xcrun swift test --disable-keychain --disable-netrc --filter 'HerdListTests|CommandPaletteIndexTests|BessieVisualFoundationTests|HerdRailPresentationTests|MenuBarPresentationTests|SettingsAndNotificationsTests|SurfaceProjectionTests/testNotification'"
ssh jordan-macbook "cd '$scratch' && xcrun swift build --disable-automatic-resolution --disable-keychain --disable-netrc -c release --product BessieApp"
```

## 2026-08-09: BES-46 independent-review repairs

- **Exact Ctrl-C:** `BessieTerminalView` emits raw ETX only when the device-independent modifier set is exactly Control. `Ctrl-Shift-C` and `Ctrl-Option-C` retain Herdr key routing as `ctrl+shift+c` and `ctrl+alt+c`; `Ctrl-Command-C` remains app-owned. Proof-first `testControlCRequiresExactModifiersAndModifiedChordsKeepTheirRouting` failed because Ctrl-Shift-C emitted raw ETX, then passed after the exact-modifier guard.
- **Truthful Copy validation:** removed `copySelectionProbePoint` and all use of libghostty's location-sensitive `selectionMenuPoint(at:)` as general selection state. Pinned `libghostty-spm` 1.3.2 exposes no public location-free `hasSelection` or selected-text query; Copy is therefore validated as an available safe no-op action, while actual selection is proved through public `performBindingAction("select_all")`, `copySelectedTextToPasteboard()`, and exact pasteboard content. `testTerminalContextMenuAndValidationExposeHonestCopyPasteStates` failed before the repair; `testSelectAllCopyUsesPublicGhosttySelectionAndExactPasteboardContent` passes against a real in-memory libghostty surface. Manual acceptance must still prove pointer drag → Edit-menu Copy and pointer drag → context-menu Copy in the live app because the pinned public API cannot expose pointer-selection state without performing Copy.
- **Right-button gesture latching:** terminal-versus-context routing is selected on right-button down and retained through drag/up, including Shift transitions. Proof-first `testTerminalRightMouseGestureIgnoresShiftAddedAfterTerminalDown` failed with only the press emitted; it and `testContextRightMouseGestureIgnoresShiftRemovedAfterDown` now pass, proving matched terminal press/drag/release and no terminal bytes for a context gesture.
- **Paste responder chain and FIFO:** `testNilTargetPasteTraversesActualWindowResponderChain` creates a real window, makes the retained terminal first responder, and calls `NSApp.sendAction(..., to: nil, ...)`; the headless XCTest host cannot establish a key window, so that test records one explicit skip rather than substituting direct dispatch. Existing direct responder, reattachment, Unicode/newline, empty pasteboard, and ordered command/responder paste tests pass; all paste operations remain `.paste` operations on the existing FIFO Herdr path.
- **Verification and installed acceptance:** a fresh Mac scratch build ran the focused terminal suite with **103 tests, 1 headless responder-chain skip, and 0 failures**, then the complete package suite with **637 XCTest cases, 6 skipped, and 0 failures** plus all **21 Swift Testing cases in 3 suites with 0 failures**. `./scripts/check.sh` and the BES-46-scoped `git diff --check` passed on the VPS. The Developer ID-signed package and `/Applications/Bessie.app` executable matched at SHA-256 `f8e0f7d49c0c4ace7311c7a19a65d5e2bcba39402862ff6cd29e483beeebe86d`; strict deep signature verification passed, the app relaunched as PID `53148`, and existing Herdr panes survived replacement. Jordan then accepted pointer selection with `Cmd-C`, multiline Unicode `Cmd-V`, terminal `Ctrl-C`, Edit-menu Copy/Paste, and terminal context-menu Copy/Paste in the installed app. No iOS implementation file, upstream dependency, commit, push, or PR was changed or performed.

## 2026-08-08: BES-19 settled-design and native-iOS feasibility plan

- **Notification product lock:** Jordan selected the end-to-end encrypted Cloudflare Worker → APNs relay for iOS V1 and instructed planning to assume it is available. Relay delivery is included at no user charge initially, while the capability/entitlement boundary must permit a future paid offering without promising perpetual free service. The watcher encrypts on the user-owned Mac/VPS to an iPhone-generated key; Bessie infrastructure may handle routing metadata and ciphertext but never receives a decryption key or plaintext Herdr content.
- **Scope decision:** Jordan removed Mosh from iOS V1. The ce-plan, roadmap, and queued goal contract now use SSH only for host shell, Herdr API, and pane streams; require honest foreground reconnect/resnapshot rather than seamless PTY roaming; and retain Mosh research only as a deferred post-V1 record.
- **Superseded notification exploration:** the earlier best-effort-only V1 position and CloudKit alternative were replaced by the product lock above. CloudKit remains research context only; the selected V1 route is the E2EE Cloudflare relay.
- **Design reconciliation:** updated the canonical iOS ce-plan, roadmap, and queued goal contract to the published 15-screen atlas. The contract now preserves Welcome → Add host → Connection check; cross-host Inbox; Pinned/Snoozed/Hierarchy popouts; one focused Shell; restricted Command; persistent five-item dock plus detached Command circle; the exact context-strip values; no redundant top/back/path bars; the final Settings inventory; no Compatibility screen/Appearance preview; and no mobile Project surface.
- **Architecture:** selected a native Swift implementation: SwiftUI for product surfaces, narrow UIKit hosting/focus/keyboard integration for `GhosttyTerminal`, shared pure-Swift `BessieCore`, and iOS-only SSH/Keychain/lifecycle adapters. React Native, web views, Catalyst, and wholesale reuse of Mac AppKit chrome are explicitly rejected.
- **Feasibility:** the atlas UI is ordinary adaptive SwiftUI work with explicit accessibility, safe-area, keyboard, iPad, and gesture gates. Exact upstream `libghostty-spm 1.3.2` declares iOS 15+, exposes `GhosttyTerminal`, and pins an XCFramework checksum, so the renderer is an integration gate rather than an unknown platform capability. A deeper dependency audit confirmed current Blink `mosh-apple` precedent but also its real Swift ↔ C ABI ↔ C++17/Protobuf boundary and continuing GPL/source obligations; that evidence now supports deferring Mosh rather than gating V1 on it. SwiftNIO SSH is viable foundation code but explicitly not a production-ready client, so M1 owns the full SSH client proof. iOS sockets may still suspend/terminate; reopening reconnects and resnapshots, while the selected user-owned watcher and E2EE Cloudflare → APNs relay provide Needs You delivery outside the app lifecycle.
- **Evidence:** the public atlas was republished and verified at HTTP 200 with Encrypted relay status/privacy copy present and stale best-effort/offline-relay/Mosh copy absent. Visual inspection confirmed the added row and note fit screen 13 without clipping or dock overlap. Its Linear comment says 15 screens and records SSH-only V1/Mosh deferral plus the selected E2EE relay/free-initially product lock. Repository `Package.swift` remains exactly pinned to `libghostty-spm 1.3.2`; the upstream manifest declares `.iOS(.v15)` plus the pinned `GhosttyKit.xcframework` checksum. Cloudflare’s current paid Workers tier is $5/month including 10 million requests and 30 million CPU-ms; relay use is nonetheless a free-to-user launch feature, not a perpetual-free promise. No app implementation, package, install, commit, push, or PR was performed.

## 2026-08-08: BES-47 Ghostty presentation compatibility vertical slice

- **Bounded nine-key profile:** the pure `BessieCore` parser recognizes only `font-family`, the four `font-style*` keys, `font-thicken`, `font-thicken-strength`, `cursor-style`, and `cursor-style-blink`. It matches Ghostty's full-line comment, outer-double-quote, boolean-alias, case-sensitive cursor, repeatable font-family/reset, queue/replay import, relative-path, and last-wins semantics. Imports are canonicalized, cycle-safe, UTF-8 validated, and bounded by depth, file count, aggregate bytes, and diagnostic-value length. Optional imports suppress missing files only. Sensitive ignored/unknown/import values are redacted; assignments retain bounded provenance and honest applied/overridden/ignored/unsupported/invalid classifications.
- **Ownership and precedence:** selected files and imports remain read-only Bessie memory. Terminal controllers receive only sanitized typed values through pinned `libghostty-spm 1.3.2` builder operations; the four unsupported style keys and `block_hollow` never use the raw/custom escape hatch. Every live transaction first emits explicit Bessie font-family, thickening, strength, cursor-shape, and cursor-blink defaults, then applies the compatibility profile, the mandatory Bessie/Herdr overlay (`mouse-reporting = false` remains final for mouse ownership), and finally the temporary pane font size. The explicit baseline prevents live Ghostty reloads from retaining imported values after disable or replacement. No shell, command, PTY, process, window, tab, split, keybinding, input, paste, or mouse-negotiation ownership moved out of Herdr.
- **Transactional state:** only enabled/disabled and the selected path are persisted; imported content, profile, effective values, diagnostics, and provenance remain in memory. Selection, enable, and disable save before publishing preferences and roll back terminal presentation on persistence failure. Pane, registry, and multi-registry transactions now distinguish successful restoration from rollback failure, retry against actual libghostty state rather than cached theme identity, and report inconsistent presentation honestly instead of claiming restoration. Invalid edits and rejected reloads retain the last-known-good profile.
- **Review and focused evidence:** the required Oracle challenge returned change-requested, identifying silently ignored pane/fleet rollback failures and incomplete persistence/parser semantics. Those findings were reproduced with red tests and fixed, including optional-import error discrimination, valid-but-unsupported last-wins behavior, Ghostty boolean/cursor rules, import reset during replay, transactional persistence, and rollback-failure reporting. Final isolated Mac scratch `/tmp/bessie-bes47-final-1786226947` passed `xcrun swift test --filter 'GhosttyCompatibilityTests|GhosttyCompatibilityIntegrationTests|ThemeTransaction|RejectedLiveControllerCandidate'`: **21 tests, 0 skipped, 0 failures**. After installed disable acceptance exposed retained Ghostty values, a focused reset test failed against the old implementation, passed after explicit baseline commands were added, and was strengthened after the final Amp review to exercise enable → disable on the same retained pane controller. The updated compatibility selection passed **19/19** and the focused transition passed unchanged.
- **Final verification and installed acceptance:** the unchanged complete `xcrun swift test` command passed **623 XCTest cases with 5 skipped and 0 failures**, followed by all **21 Swift Testing cases in 3 suites with 0 failures**. Before that final pass, one full run exposed and drove a real rollback revalidation fix: the pinned builder rounds font size to two fractional digits, so the prior `+0.001` probe did not produce a distinct config source; `+0.01` is now the smallest effective probe. Existing Swift 6 main-actor isolation warnings from `BessieVisualFoundationTests` remain warnings. VPS `./scripts/check.sh` and BES-47-focused `git diff --check` pass. The final Developer ID-signed package and `/Applications/Bessie.app` executable matched at SHA-256 `03b783fcc0e02c9aa1204e2ad3c13581bbf0c0dd496aa65e015adcc27dc4e915`; strict deep signature verification passed. The installed fixture produced the expected applied/ignored diagnostics, its forbidden `command` sentinel was not created, and Jordan confirmed that enabling changed the font/thickness/cursor while disabling restored all three to Bessie's normal presentation. Public Herdr inventory retained pane and terminal identities across app replacement. Source remains uncommitted and unstaged; no push, publication, Herdr/libghostty upstream source change, or BES-3 update-release-state change was performed.

## 2026-08-08: BES-48 hierarchy-filter persistence — installed acceptance

- **Cause and persistence boundary:** the manually selected Herd / Workspace / Tab hierarchy scope lived only in `ProductSurfaces` SwiftUI state. `BessiePresentationState` now stores a dedicated Codable scope preference for selected-tab, all-tabs, all-workspaces, and all-herds modes; legacy files without the field continue to decode. `BessieSettingsModel` restores and persists the value through the existing atomic presentation store rather than copying Herdr topology.
- **Fresh-snapshot reconciliation:** startup restores the persisted hint before initializing a default. Revalidation preserves unresolved identifiers while their connection is not fresh, then applies the documented ladder against a fresh Herdr snapshot: missing tab to All Tabs, missing workspace to All Workspaces, and missing or disabled connection to All Herds. Reconciliation runs on both topology-identity and connection-freshness transitions, updates only Bessie selection/presentation state, and does not invoke Herdr focus or topology mutation.
- **Tests and review:** the new persistence test failed before implementation. The focused Mac `WorkspaceScope` selection passed **6/6** after the final freshness-transition fix. The complete Mac Swift suite passed **605 tests with 5 skipped and 0 failures** before that final observer addition; the changed source recompiled in the passing focused rerun. VPS `./scripts/check.sh` and `git diff --check` passed. Two independent read-only reviews were completed; the first found the missing freshness-transition reconciliation, and the second passed the complete six-file implementation after the fix with no security concerns, logic errors, or suggestions.
- **Production install identity:** source and tests were committed as `069648f` (`fix: persist hierarchy filters across relaunch`). The first Developer ID signing probe correctly failed under headless SSH with `errSecInternalComponent`; the same repository script was then run inside the logged-in Mac GUI session and completed successfully. The packaged and installed executables both had SHA-256 `274675408db9f0a595435a19155882b8e5f8610fc94cf24d383d964d80a7cf43`, `cmp` and strict deep signature verification passed, the bundle remained `dev.bessie.app`, and the installed app retained Team ID `T4K7A3GPQ6`.
- **Installed relaunch proof:** in the signed installed app, the manual scope was changed to **All Herds / All Workspaces / All Tabs**. The production presentation file recorded `workspace_scope = {"kind":"all_herds"}`. A full quit removed PID `86166`; relaunch created PID `87533`. The persisted file still contained `all_herds`, and an inspected post-relaunch screenshot showed the exact three labels **All Herds**, **All Workspaces**, and **All Tabs**, populated pane rows, and live terminal output with no empty-state or visible error regression. Public Herdr pane reads before and after replacement/relaunch retained panes `w2E:p2Y` and `w2E:p35` with the same terminal IDs; Bessie did not terminate or recreate the Herdr server or panes. Missing-resource fallback remains covered by focused model tests rather than destructive live-topology mutation. No push, publication, Herdr/libghostty change, or update-release-state change was performed.

## 2026-08-08: Catppuccin semantic fidelity — U1 canonical source checkpoint

- **Source:** added an immutable named Catppuccin v1.8.0 palette source covering all 26 colors across Latte, Frappé, Macchiato, and Mocha. Existing native Catppuccin palette construction now references that source rather than repeating canonical hexadecimal literals.
- **Preservation:** this checkpoint intentionally leaves semantic assignments and rendered native values unchanged. The independently pinned Ghostty terminal tables are untouched; Bessie Dark, Bessie Light, and System behavior retain exact baseline fingerprints.
- **Tests:** new contracts prove all 104 canonical colors, opaque alpha, complete flavor/name coverage, exact Bessie Dark/Light RGBA fingerprints, and System's Bessie-only resolution. VPS `./scripts/check.sh` and `git diff --check` passed. After a corrected relative-path mirror sync, Mac `xcrun swift test --filter BessieThemeTests` rebuilt the changed source and passed **20/20** tests.
- **Status:** U1 is complete; semantic role remapping, caller migration, provenance, visual acceptance, packaging, install, and credits remain. No commit, push, PR, Herdr/libghostty change, or full `mac-verify.sh` run was performed.

## 2026-08-08: Catppuccin semantic fidelity — U2 role-contract checkpoint

- **Semantic contract:** expanded the native palette boundary with explicit active-border, destructive, link, control-tint, and insertion-point intents. All four Catppuccin flavors now derive native chrome from the canonical named source: Crust/Mantle/Base/Surface foundations; Text/Subtext/Overlay copy; Overlay interaction treatments; Lavender active borders; Blue links/controls; Rosewater insertion point; Red destructive state; and Green/Red/Peach diff identities. Bessie Dark and Bessie Light retain exact native fingerprints, including their existing macOS system-red behavior, and their Ghostty definitions remain unchanged.
- **Accessibility:** Latte's bounded derivatives are named, source-linked, and measured without altering canonical constants. The active-border derivative is tested against the actual Overlay 2-at-25%-over-Surface 0 selected-panel composite, not an easier Base background. Its encoded target and all text/diff targets are executable contracts with hue/chroma bounds and explicit fallback policy coverage.
- **Verification:** VPS `./scripts/check.sh` and `git diff --check` passed during implementation. On the Apple Silicon Mac, `xcrun swift test --filter BessieThemeTests` passed **25/25** and `xcrun swift test --filter BessieVisualFoundationTests` passed **26/26**. Coverage includes all 104 canonical colors, semantic mappings, interaction distinctness, same-polarity mounted updates, terminal-controller identity, exact terminal preservation, accessibility derivatives, and the frozen non-Catppuccin palettes.
- **Status:** U2 is complete. Native caller migration, attribution/credit, live capture, packaging, install, screenshot inspection, and installed executable identity remain. No full `mac-verify.sh`, commit, stage, push, PR, Herdr/libghostty edit, or installed-app mutation was performed.

## 2026-08-08: Catppuccin semantic fidelity — U3 caller-intent checkpoint

- **Caller migration:** removed broad root-level strong tint inheritance and assigned control tint, insertion point, link, running/progress, selection, active-border, destructive, and diff roles at their owning native controls. Selected fills no longer receive caller-owned duplicate opacity; selected pane/card outlines use the active-border role; diagnostic and brand-fixed colors remain intentionally bounded.
- **Scope correction:** an implementation worker widened four plausible styling edits into `ProjectEditorView.swift`, `ProjectsSurface.swift`, `WorkspaceFilesSurface.swift`, and `WorkspaceTitlebarChrome.swift`. Those edits were rejected and restored before acceptance. The retained caller boundary is `BessieApp.swift`, `BessieCommandPalette.swift`, `BessieDesignSystem.swift`, `BessieSettings.swift`, `FollowFilesSurface.swift`, `MarkdownFileEditor.swift`, `OnboardingView.swift`, `ProductSurfaces.swift`, and `RuntimeSettingsView.swift`.
- **Executable audit seam:** `BessieVisualFoundationTests` now asserts removal of broad strong tint, direct system semantic colors, duplicate selected opacity, and role leakage while requiring explicit local control/caret/link/progress/destructive usage.
- **Verification:** after restoring the rejected files, the coherent isolated Mac command `xcrun swift test --filter "BessieVisualFoundationSourceTests|BessieThemeTests|BessieVisualFoundationTests"` executed **53 tests with 0 failures**; the source-audit tests are methods inside `BessieVisualFoundationTests`. VPS `git diff --check` passed. `./scripts/mac-verify.sh` was not run.
- **Status:** U3 is complete. No stage, commit, push, PR, Herdr/libghostty source change, or terminal-palette change was performed.

## 2026-08-08: Catppuccin pre-commit review closure — waived U4 rerun checkpoint

- **User decision:** Jordan explicitly chose **“Commit the finalized theme changes now and begin BES-47”**, waiving the remaining signed install and visual acceptance rerun for this checkpoint. This review closure did not sign, package, install, launch Bessie, capture a screenshot, mutate real Bessie preferences, or modify Herdr/libghostty.
- **Evidence boundary:** the U1 canonical-source, U2 role-contract, and U3 caller-intent evidence above remains valid and is preserved. U4 is **not claimed complete**: no packaged/installed Latte or Mocha screenshot was captured during this review closure, no region-evidence manifest was reviewed, and `CATPPUCCIN_ACCEPTANCE_OK` was not emitted.
- **Acceptance contract only:** the focused wrapper now installs without the helper's pre-acceptance relaunch, owns and revalidates every automated installed-app PID through bounded shutdown, checks the real presentation digest only after all automated apps stop, sends a distinct Unicode token through public `pane.send_input` with `keys:["Enter"]` separately from the ANSI marker, and defines deterministic Latte/Mocha interaction and menu-bar captures. Its generated 42-region manifest covers all four Settings and terminal seams, starts with explicit failures, and is bound to screenshot plus installed Bessie/Herdr digests; success is available only in no-launch `--verify-evidence` mode after every required region is changed to an explicit reviewed pass.
- **Actual checks:** VPS `./scripts/check.sh` passed (including the 9-file native color audit, independent 9-caller inventory contract, violation fixtures, and fail-closed region-manifest self-test); explicit Bash syntax checks passed; Python byte-compilation passed; the focused contract self-tests passed; both print-configuration paths passed; and repository-wide `git diff --check` passed. A fresh isolated Mac scratch at `/tmp/bessie-theme-compile.38twbv` compiled the complete source, including the deterministic preview hook, and `xcrun swift test --filter "BessieThemeTests|BessieVisualFoundationTests"` passed **53 tests with 0 failures** (25 theme and 28 visual-foundation tests). This compile/test did not package, sign, install, launch Bessie, capture screenshots, or mutate user state. No Mac-only or signed acceptance command was run.
- **Remaining blocker by waiver:** U4 packaged/installed visual and human region evidence still requires an explicitly authorized signed Mac rerun. Until then, the implementation must not be described as visually accepted, installed-current, or U4-complete.
...
- At this checkpoint, the Herd presented Needs you (`blocked`), Working (`working`), Settled (`done` or `idle`), and Unknown (unknown or unrecognized), with All retained as an aggregate filter. The synthetic Settled presentation was superseded by the 2026-08-09 raw Done/Idle status contract above.
...
- Reconciled roadmap and older plan authority at this checkpoint: menu bar and entity palette are Pre-v1, the newer floating-card shell/four-step onboarding supersedes older visuals, elapsed-age UI is absent, and `Cmd+B` / empty-selection `Cmd+C` remain one-shot terminal mappings rather than a Bessie prefix mode. Its four-state Settled vocabulary was superseded by the 2026-08-09 raw Done/Idle status contract above.
...
- **Subsequent production acceptance:** Jordan authorized local integration. Commit `e46b4e2` was fast-forwarded onto `feat/v1-l-brand-chrome`; a fresh complete Mac run passed **590 tests with 5 skipped and 0 failures**. Jordan then used the interactive GUI login to produce a Developer ID-signed production package with bundle identifier `dev.bessie.app`. The first installer attempt stopped before replacement because `grep -Fq` closed a verbose `codesign` pipe early under `pipefail` (exit 141). Follow-up commit `774edd2` captures signature metadata before matching it, and both signature assertions and VPS checks pass.
- The signed package was transactionally installed at `/Applications/Bessie.app`. Packaged and installed `BessieApp` SHA-256 values both equal `628565cf15d127eff97e9e50dcf2b39eb630bc5bb8d68006b9b857d8be08d727`; strict deep verification reports `Developer ID Application: JORDAN JAMES STELLA (T4K7A3GPQ6)` and Team ID `T4K7A3GPQ6`. Bessie relaunched, its public intent bus reported `running=true`, the selected `hermes-vps` connection reported `connected=true`, and the pre-existing installed Herdr server PID survived app replacement before the relaunched app reattached.
- **Visible notification proof:** Bessie reported production notification permission as granted and macOS `usernoted` recorded the test request as delivered, but the first test showed no banner. The active Focus database contained a live Do Not Disturb assertion and did not allowlist `dev.bessie.app`, explaining delivery without presentation. After Jordan disabled Do Not Disturb and retried, he confirmed the test banner appeared; clicking it focused Bessie and routed to the expected pane. Policy transition behavior, stale Notification Center cleanup, and notarization remain unclaimed.

## 2026-08-09: BES-3 U1 Sparkle packaging foundation

- Added the exact SwiftPM dependency `https://github.com/sparkle-project/Sparkle` `2.9.5` and attached its `Sparkle` product only to `BessieApp`. `Package.resolved` pins revision `79bc9e872948e47877e76f194cb0c8e0412b0b90`; the resolved SwiftPM workspace metadata records archive checksum `34b9b2071f3de0012eca3faa3a9290bb94e62131e9a74f6dc91514a000097a6c`. Release builds disable automatic resolution.
- The packaging path now selects exactly one Sparkle artifact from repository-local `.build/workspace-state.json`, cross-checks package identity, target, URL, checksum, lockfile version/revision, XCFramework platform/architectures, containment, code identifiers, arm64 support, original signature, and install name, then copies only `Sparkle.framework` with `ditto`. It requires the exact 2.9.5 `Versions/B` helper/XPC and symlink layout and rejects flattened links, extra XPC services, tools, `BinaryDelta`, old DSA scripts, and dSYMs.
- The package executable has the exact `@rpath/Sparkle.framework/Versions/B/Sparkle` load command and `@executable_path/../Frameworks` runpath; package validation rejects build/cache runpaths. Sparkle's Updater, Autoupdate, Installer, Downloader, framework binary, and framework bundle are signed explicitly inside-out while preserving identifiers, existing entitlements, and identifier-based designated requirements. `codesign --deep` remains verification-only. Production verification requires Developer ID Application authority, one team, hardened runtime, and secure timestamps for bundled Herdr, every Sparkle subject, and the outer app.
- Production plist rendering now requires explicit numeric marketing/build versions, exact HTTPS feed `https://bessie.dev/appcast.xml`, a canonical nonzero 32-byte Ed25519 public key, and a separately approved matching public-key SHA-256. The final rendered plist is revalidated for exact signed-feed/pre-extraction/failure-expiration/automatic/privacy settings and absence of `SUAllowsInsecureUpdates`. Ad-hoc `verify` packages contain no active `SU*` configuration and cannot contact a feed.
- **RED:** after adding the focused test first, `./scripts/test-sparkle-packaging.sh` exited 1 at line 8 because `scripts/lib/sparkle-packaging.sh` did not exist. The GREEN test now covers exact target scoping; lock/workspace/checksum binding; ambiguity and malformed framework failures; all required symlinks/helpers; excluded artifacts; production version/feed/key/fingerprint rejection; rendered plist policy; verify-plist omission; and Developer ID authority/team/runtime/timestamp rejection.
- **VPS verification:** `./scripts/test-sparkle-packaging.sh`, `./scripts/check.sh`, `bash -n scripts/package-app.sh scripts/lib/sparkle-packaging.sh scripts/test-sparkle-packaging.sh`, and the U1-scoped `git diff --check` all exited 0. Swift remains unavailable on the VPS.
- **Fresh Mac scratch verification:** created detached scratch `/tmp/bessie-bes3-u1-head-1786241804` from VPS HEAD and synced only the U1 files with no deletion. SwiftPM resolved Sparkle `2.9.5`; `xcrun swift build --disable-keychain --disable-netrc -c release --product BessieApp` passed. `xcrun swift test --disable-keychain --disable-netrc --filter BessieThemeTests` compiled the package and passed **26 tests with 0 failures**. The final `BESSIE_PACKAGE_VARIANT=verify BESSIE_CODESIGN_IDENTITY=- ./scripts/package-app.sh` passed, including source-artifact validation, symlink-preserving embedding, explicit nested ad-hoc signing, bundled Herdr checks, and strict deep verification.
- **Runtime/structural proof:** copied the package with `ditto` to an isolated `/tmp` app outside its build tree, verified every Sparkle code subject and the outer bundle, launched it directly for five seconds with `DYLD_PRINT_LIBRARIES=1`, and proved dyld loaded `Contents/Frameworks/Sparkle.framework/Versions/B/Sparkle` from that isolated app. The launched scratch process was terminated and the temporary isolated app/log were removed. Runtime linkage is therefore not deferred to U2.
- Oracle identified fail-open artifact scanning, key approval, final-plist, Developer ID, source-code, and cache-rpath gaps before finalization; the implementation above closes them. The focused test was not added to `scripts/check.sh` because the authorized U1 write set did not include that shared gate; it was run explicitly on both Linux and macOS.
- **Deferred/operator-gated evidence:** no production public key/fingerprint was invented; no production package, Developer ID signing, notarization, Gatekeeper assessment, updater startup/lifecycle/UI, old-to-new update, installation, publication, deployment, or credential use was attempted. Developer ID enforcement is fixture-tested but requires a later approved real identity run. No app was installed, and `./scripts/mac-verify.sh` was not invoked.
- U1 changed only `Package.swift`, `Package.resolved`, `scripts/Info.plist.in`, `scripts/package-app.sh`, `scripts/lib/sparkle-packaging.sh`, `scripts/test-sparkle-packaging.sh`, and this appended report entry. Accepted BES-46 and unrelated iOS plan/report/source/test changes remain untouched. No stage, commit, push, PR, reset, stash, clean, publish, deploy, install, production signing, or Herdr-owned runtime mutation was performed.
- **Parent verification:** a second fresh Mac scratch at `/tmp/bessie-bes3-u1-parent.vvAbrP` independently reran the focused packaging test and produced an ad-hoc `dev.bessie.app.verify` package. Strict verification passed for every nested Sparkle code subject and the outer app; `Versions/B` and all public framework symlinks remained intact; the executable carried the exact Sparkle load command and app-relative Frameworks runpath; and a direct scratch launch loaded the bundled `Versions/B/Sparkle` image before the exact scratch PID was terminated. Public Herdr panes `w2E:p2Y` and `w2E:p3G` remained present. No installed app was replaced.

## 2026-08-09: BES-3 U2 app-lifetime updater coordinator

- Added a main-actor `BessieUpdateCoordinator` with Bessie-owned value state, one narrow injected adapter, one retained `SPUStandardUpdaterController(startingUpdater: false, updaterDelegate: ..., userDriverDelegate: nil)`, and Sparkle 2.9.5's throwing `SPUUpdater.start()` API. The coordinator is not wired into `BessieApp` yet; that remains U3.
- Production eligibility requires an `.app` bundle with exact identity `dev.bessie.app`. Unpackaged development binaries, XCTest bundles, and `dev.bessie.app.verify` are ineligible and do not create an updater. The explicit update-test contract requires both `BESSIE_UPDATE_TEST_MODE=1` and a credential-free HTTPS feed or loopback-only HTTP feed; no environment override affects the production feed outside that contract.
- Sparkle remains authoritative for automatic-check and automatic download/install preferences. The coordinator reads and writes only public `SPUUpdater` properties and observes all three KVO-compliant values, including `allowsAutomaticUpdates`, so Sparkle's own permission UI can refresh Bessie's published value state without a second preference store.
- Ready-to-restart state is created only by `updater(_:willInstallUpdateOnQuit:immediateInstallationBlock:)`. Exact short/build metadata and the closure are retained on the main actor; ownership returns `true` only after retention; duplicate callbacks replace rather than multiply the handler; invocation clears the handler before calling it; and no Bessie code terminates the app. Authorization cancellation remains consumed under the frozen one-shot rule until an independent future Sparkle cycle supplies another handler.
- Status handling preserves Sparkle's complete no-update callback sequence as quiet background success or visible manual “up to date” state, clears transient found-update status on ordinary cycle completion, distinguishes background/manual/startup failure contexts, never stores raw errors, and uses fixed sanitized text without paths, URLs, credentials, payloads, or process output.
- **RED 1:** in fresh Mac scratch `/tmp/bessie-bes3-u2-red-1786244289`, `xcrun swift test --disable-keychain --disable-netrc --filter BessieUpdateCoordinatorTests` failed to compile because every new coordinator/adapter/state symbol was absent. **RED 2:** after review added the finished-found-update regression, `xcrun swift test --disable-keychain --disable-netrc --filter BessieUpdateCoordinatorTests/testFinishedFoundUpdateWithoutInstallOnQuitReadinessClearsTransientStatus` executed one test with one expected failure because `Preparing Bessie 1.2.0…` remained stale.
- **Final fresh Mac proof:** in separate scratch `/tmp/bessie-bes3-u2-green-1786245811`, `xcrun swift test --disable-keychain --disable-netrc --filter BessieUpdateCoordinatorTests` passed **16 tests with 0 failures**. The U2 source and test files emitted **0 warnings** under Swift 6. The fresh package compile still emitted pre-existing actor-isolation warnings from `BessieVisualFoundationTests`, outside the authorized U2 files.
- **Parent verification:** the final U2 source and tests were independently synced into `/tmp/bessie-bes3-u1-parent.vvAbrP`. `xcrun swift test --disable-keychain --disable-netrc --filter BessieUpdateCoordinatorTests` again passed **16 tests with 0 failures**, and `xcrun swift build --disable-automatic-resolution --disable-keychain --disable-netrc -c release --product BessieApp` exited 0. The only emitted warning was the pre-existing `HerdRail.swift:358` `NSApp` coercion warning; the U2 source emitted none.
- `xcrun swift build --disable-automatic-resolution --disable-keychain --disable-netrc -c release --product BessieApp` completed in **71.81 seconds** in the same fresh scratch, compiling the production Sparkle adapter against the resolved official 2.9.5 declarations with **0 U2 warnings**. No updater was started and no appcast/feed request occurred in tests.
- VPS `./scripts/check.sh` exited 0, including runtime-lock, intent-parity, theme, UI-copy, and static checks. U2-scoped `git diff --check` exited 0 before this append. No `Package.swift` test dependency change was required.
- U2 changed only `Sources/BessieApp/BessieUpdateCoordinator.swift`, `Tests/BessieAppModelTests/BessieUpdateCoordinatorTests.swift`, and this append-only report entry. Existing U1, BES-46, and iOS changes remain untouched. No `mac-verify.sh`, package, install, sign, notarize, commit, push, PR, publication, deployment, credential use, app termination, or Herdr-owned runtime mutation was performed.

## 2026-08-09: BES-3 U3 updater lifecycle, command, and General Settings

- `BessieApp` now owns exactly one app-lifetime `@StateObject` update coordinator, starts it once, and injects the same instance into the product and Settings scenes. The existing single-window policy and quit command remain intact. The native app menu now includes **Check for Updates…**, backed by that coordinator.
- General Settings now has an Updates block for Sparkle-owned automatic checks and automatic download/install preferences, current app version/build, target version/build and bounded sanitized status when present, plus the manual check action. Update controls call only coordinator/Sparkle public preferences and do not persist through or rewrite `BessieSettingsModel` presentation data.
- The authorized narrow coordinator amendment publishes Sparkle 2.9.5's live KVO-compliant `SPUUpdater.canCheckForUpdates` capability. Availability begins false until the retained updater starts, follows live false/true transitions, and is combined with coordinator phase eligibility. The manual action synchronously re-reads the adapter's live capability immediately before invoking Sparkle, closing the delayed-KVO stale-true window and preventing a false-enabled check.
- Strict RED→GREEN evidence was retained across three test-first checkpoints in fresh Mac scratch `/tmp/bessie-u3-red-1786246520`: the initial U3 tests failed because lifecycle/command/Settings types were absent; the live-availability tests failed to compile because the adapter delegate had no availability callback; and the stale-live-capability regression executed with three expected failures because the old coordinator invoked the adapter and entered manual checking. After implementation, the final focused selection passed **25 tests with 0 failures**.
- Broader fresh-scratch command `xcrun swift test --filter 'BessieUpdate|SettingsAndNotificationsTests'` passed **63 tests with 0 failures**. `xcrun swift build -c release` exited 0; the only release warning was the pre-existing `HerdRail.swift:358` optional `NSApp` coercion warning. `./scripts/check.sh` exited 0 with runtime-lock, intent-parity, theme, UI-copy, and static checks passing. Scoped tracked and untracked `git diff --check` checks exited 0. Independent final review approved U3 with no blockers.
- **Parent verification:** the final repository tree was independently synced without deletion into `/tmp/bessie-bes3-u1-parent.vvAbrP`. `xcrun swift test --disable-keychain --disable-netrc --filter 'BessieUpdate|SettingsAndNotificationsTests'` again passed **63 tests with 0 failures**, and `xcrun swift build --disable-automatic-resolution --disable-keychain --disable-netrc -c release --product BessieApp` exited 0. The U3 source emitted no warnings; only the known `HerdRail.swift:358` and unrelated `BessieVisualFoundationTests` actor-isolation warnings appeared.
- Termination code was not changed. Focused lifecycle assertions prove the ordinary hooks still stop/release only Bessie client, intent, fleet, project, and terminal-registry surfaces; the scoped ownership scan found no production `herdr stop`, pane/tab/workspace close, process kill, or app-owned PTY behavior. No Herdr state or process was mutated.
- U3 implementation/test changes are limited to `Sources/BessieApp/BessieApp.swift`, `Sources/BessieApp/BessieSettings.swift`, `Sources/BessieApp/BessieUpdateCoordinator.swift`, `Tests/BessieAppModelTests/SettingsAndNotificationsTests.swift`, `Tests/BessieAppModelTests/BessieUpdateCoordinatorTests.swift`, `Tests/BessieAppModelTests/BessieUpdateLifecycleTests.swift`, and this append-only report entry. `Sources/BessieApp/BessieAppDelegate.swift` and all U4 rail files were untouched; unrelated accepted BES-46, iOS, U1, and U2 work remains preserved.
- Programmatic native Settings screenshot evidence was not captured because doing so without the accepted installed app would risk replacing it or disrupting Jordan's desktop. Combined launched dark/light Settings acceptance is deferred to U4/final verification. Per scope, `mac-verify.sh`, packaging, installation, signing, notarization, commit, push, PR, publication, deployment, credentials, and production feed activity were not invoked.

## 2026-08-09: BES-3 U4 expanded and collapsed Restart to Update rail action

- The product shell now derives and passes only a Bessie-owned `HerdRailUpdatePresentation` value plus the coordinator's restart closure into `HerdRail`; no Sparkle updater or raw installation handler crosses the view boundary. Presentation exists only for `.readyToRestart` while the coordinator confirms its retained one-shot handler still exists. Idle, unverified ready, and installing states expose no rail action.
- The update row is driven by an explicit footer order of update action, Settings divider, then Settings. Expanded copy is exactly **Restart to Update** with the target display version as secondary text; collapsed accessibility and both-density help are exactly **Restart to Update Bessie <version>**. The clockwise-arrow symbol supplies a non-color cue, semantic theme roles preserve Catppuccin and Increase Contrast behavior, hover uses the existing semantic hover role, and no motion was added. Existing 244/52 rail geometry remains unchanged.
- A rail-local activation gate consumes before calling the restart closure and disables re-entry until the updater presentation changes. The accepted U2 coordinator independently clears its retained handler and enters installing before invoking Sparkle, so repeated activation cannot reuse a consumed handler; only a fresh later install-on-quit callback can restore readiness. No termination, Herdr mutation, or pre-relaunch success state was added.
- **Strict RED:** tests were added first and run in fresh Mac scratch `/tmp/bessie-u4-red-1786248552`. `xcrun swift test --disable-keychain --disable-netrc --filter HerdRailPresentationTests` failed to compile on the expected missing U4 presentation, footer-order, and activation-gate symbols.
- **Fresh Mac GREEN:** in `/tmp/bessie-u4-green-1786248677`, `xcrun swift test --disable-keychain --disable-netrc --filter 'HerdRailPresentationTests|BessieUpdateCoordinatorTests'` passed **25 tests with 0 failures**. A second focused geometry/theme/accessibility command, `xcrun swift test --disable-keychain --disable-netrc --filter 'BessieVisualFoundationTests|HerdRailPresentationTests'`, passed **34 tests with 0 failures**. `xcrun swift build --disable-automatic-resolution --disable-keychain --disable-netrc -c release --product BessieApp` passed; the only warning was the pre-existing `HerdRail.swift` optional `NSApp` coercion warning (line shifted from 358 to 409 because of the U4 additions).
- **Parent verification:** the final U4 tree was independently synced without deletion into `/tmp/bessie-bes3-u1-parent.vvAbrP`. `xcrun swift test --disable-keychain --disable-netrc --filter 'BessieVisualFoundationTests|HerdRailPresentationTests|BessieUpdateCoordinatorTests'` passed **53 tests with 0 failures**, and the release `BessieApp` build exited 0. The only production warning remained the pre-existing `HerdRail.swift:409` optional `NSApp` coercion warning; unrelated visual-foundation actor-isolation warnings remained test-only.
- VPS `./scripts/check.sh` and U4-scoped `git diff --check` passed. Independent Oracle review approved the U4 diff with no blockers and confirmed the Bessie-owned boundary, exact visibility predicate and labels, footer position, one-shot layering, and absence of direct termination.
- U4 changed only `Sources/BessieApp/ProductSurfaces.swift`, `Sources/BessieApp/HerdRail.swift`, `Tests/BessieAppModelTests/HerdRailPresentationTests.swift`, and this append-only report entry. Accepted U1-U3, BES-46, iOS, and other unrelated working-tree changes remain preserved.
- Deterministic launched rail and deferred General Settings screenshots were not captured because the accepted app could not be safely replaced or launched with a staged-update fixture without packaging/installing or disrupting Jordan's desktop. Combined expanded/collapsed light/dark launched acceptance remains deferred to final verification. `mac-verify.sh`, packaging, installation, signing, notarization, commit, push, PR, publication, deployment, credentials, and Herdr-owned runtime state were not invoked or changed.

## 2026-08-09: BES-3 U5 fail-closed release preparation and verification tooling

- Added explicit `prepare`, secret-free `verify`, and refusing `publish` boundaries in `scripts/release-app.sh`. Credentialed preparation requires a clean approved full source commit, matching local `v<marketing-version>` tag, monotonic numeric build and non-decreasing marketing version against a signed staged/deployed appcast, exact credential-free HTTPS `bessie.dev`/immutable GitHub URLs, macOS 14.0, an exact Developer ID identity/team, a usable named notary Keychain profile, and a Keychain-backed Sparkle account whose public key matches a separately approved SHA-256. Initial release requires an explicit flag; otherwise the signed previous feed and latest immutable archive are mandatory and cryptographically verified.
- Production preparation resolves Apple trust-boundary tools by fixed system paths, uses `umask 077`, and freshly downloads Sparkle's official 2.9.5 SwiftPM distribution into private scratch. It requires frozen SHA-256 `34b9b2071f3de0012eca3faa3a9290bb94e62131e9a74f6dc91514a000097a6c` before executing only `generate_keys`, `generate_appcast`, and `sign_update`. No private-key argument/file/export, deprecated `-s`, DSA, custom crypto, removed CLI, environment dump, or raw credential-bearing evidence is used or retained.
- The final byte order is package/nested Developer ID verification; metadata-preserving notarization ZIP; `xcrun notarytool submit --wait`; mandatory redacted log whose job/status/SHA binds to the local submission ZIP; staple/validate; Gatekeeper and strict code-sign checks; final metadata-preserving ZIP from stapled bytes; extraction and repeat identity/team/plist/staple/Gatekeeper verification; SHA-256/length; Sparkle appcast generation/signing with `--maximum-deltas 0 --maximum-versions 0`; and supported Sparkle archive/feed verification. App seals and repeated archive hashes reject post-staple or post-finalization mutation.
- `scripts/release-metadata.py` validates every retained feed item, exact tag/version/archive relationship, full ZIP MIME/type, positive length, canonical 64-byte signature metadata, publication date, release notes, minimum OS, stable-channel-only policy, no deltas, no credential/private/insecure URL, no reused/dropped/rewritten immutable item, archive/checksum/appcast hashes, signed-feed trailer length, and internally consistent redacted release evidence. Offline verification is explicitly documented as structural consistency against a separately trusted unsigned manifest, not independent proof of Ed25519 authenticity, notarization, code identity, stapling, or execution order.
- `scripts/package-app.sh` accepts an optional exact expected team for the release path. `scripts/check.sh` now runs the U1 Sparkle packaging suite, U5 fixture suite, shell syntax, and Python syntax checks. `docs/releases/README.md` documents trusted-Mac key/profile setup, non-secret invocation, immutable release/rollback boundaries, fresh-scratch verification limits, and the complete R31 evidence template. `mac-verify.sh`, `Info.plist.in`, and `.gitignore` required no U5 change.
- **Strict RED:** after adding `scripts/test-release-app.sh` first, the test failed because `scripts/release-metadata.py` did not exist. The final fixture suite covers malformed/tag-mismatched versions, non-increasing build, marketing-version regression, reused tag, missing/mismatched previous archive, bad/private URL, bad archive length/hash, insecure/deprecated plist policy, deltas, missing/locked identity, unusable notary profile, missing Sparkle key, notarization rejection, missing/mismatched notary log binding, post-staple mutation, previous-signature rejection, exact official CLI argument shapes, four retained prior items without pruning/URL rewrite, signing only after byte finalization, and the refusing publication boundary.
- **VPS verification:** `./scripts/test-release-app.sh`, `./scripts/test-sparkle-packaging.sh`, `./scripts/check.sh`, `bash -n` on all touched/relevant shell scripts, `shellcheck -x` on the U5/check/package scripts, Python AST parsing, scoped `git diff --check`, and the secret-free prepared-directory verifier all exited 0. Swift remains unavailable on the VPS.
- **Fresh Mac scratch:** synced the accepted working tree without deletion to `/private/tmp/bessie-bes3-u5-1786251509`. The final U5 fixture suite and Sparkle packaging suite exited 0; a fresh SwiftPM resolve selected Sparkle 2.9.5; `xcrun swift build --disable-automatic-resolution --disable-keychain --disable-netrc -c release --product BessieApp` completed in 78.31 seconds. The only warning was the accepted pre-existing `HerdRail.swift:409` optional `NSApp` coercion warning. Final synced shell syntax and release help checks also exited 0. No production package was created.
- Independent Oracle review first found retained-archive URL rewriting, unauthenticated ignored-cache tools, weak notary/final-ZIP binding, overstated offline claims, incomplete retained-feed validation, and non-initial fixture gaps. Those were corrected. A second review found default three-version pruning; preparation now uses `--maximum-versions 0`, compares every previous build/URL after generation, and fixtures four prior items. Final review returned **approved**, with only the intentionally deferred real credentialed operator run unverified.
- **Deferred operator gate:** no real Developer ID identity, notary profile, Sparkle private key, Apple submission, stapling ticket, Gatekeeper production result, or non-published credentialed prepare was used in this worker. That real trusted-Mac prepare/notary run requires separate parent authorization and execution. `./scripts/mac-verify.sh` was not run. No app was packaged for production, installed, relaunched, published, uploaded, deployed, committed, pushed, or exposed; no Herdr-owned objects were changed.
- **Parent verification:** the accepted U5 tree was independently checked on the VPS with `./scripts/check.sh`, `shellcheck -x` over the release/check/package scripts, Python bytecode compilation, and scoped `git diff --check`; all exited 0. It was then synced without deletion or credential files to the established parent Mac scratch `/tmp/bessie-bes3-u1-parent.vvAbrP`. There, the U5 release fixture suite and U1 Sparkle packaging suite passed, shell syntax checks passed, SwiftPM resolved exact Sparkle 2.9.5, and a fresh release `BessieApp` build completed successfully. The only warning was the accepted pre-existing `HerdRail.swift:409` optional `NSApp` coercion warning. No credentialed release action or external publication occurred.

## 2026-08-09: BES-3 U6 fail-closed `bessie.dev` appcast endpoint

- Imported the complete `site/` tree from `feat/bessie-dev-landing` commit `6d03a78` without merging, cherry-picking, staging, or committing. A final blob comparison proves every baseline file remains byte-identical except the intentional U6 edits to `site/README.md`, `site/package.json`, `site/package-lock.json`, `site/worker.js`, and `site/wrangler.toml`; all original `site/public/` landing assets and `site/scripts/{smoke,visual-proof}.mjs` remain exact.
- Reserved exact `/appcast.xml` before `/install` and static/SPA fallback. Cloudflare Static Assets now runs the Worker first for `/appcast.xml` and `/install`. The machine endpoint accepts only GET/HEAD, canonicalizes its internal asset lookup independently of cache-busting queries, requires a valid XML asset response, validates at most 1 MiB of strict UTF-8 well-formed RSS through `fast-xml-parser` `5.10.1`, and returns the exact bytes as `application/xml; charset=utf-8` with `X-Content-Type-Options: nosniff` and `public, max-age=300, must-revalidate`. Missing, wrong-content-type, malformed, unsigned, or policy-invalid feeds return non-HTML `503` with `no-store`; unsupported methods return `405`.
- Feed checks cover canonical signed-feed trailer shape/length, canonical 64-byte feed and enclosure signature metadata, internal title/version/build/archive-name consistency, positive enclosure length, exact macOS `14.0`, publication date, U5-compatible linked or description release notes, full ZIP MIME, exact credential-free immutable GitHub release URL text, duplicate items, private-key markers, stable-channel-only policy, and no deltas, rollout, critical, or informational items. DTD/entity declarations, malformed nesting/attributes, alternate Sparkle namespace prefixes, nested namespace rebinding, and numeric-character-reference namespace bypasses fail closed. Public/non-draft/immutable release state and actual archive bytes remain separate operator checks rather than claims inferred from XML.
- Added `npm run stage:appcast -- PREPARED_DIRECTORY`. It reruns the existing secret-free U5 `release-metadata.py verify` against the complete prepared directory, cross-checks the selected latest feed item and appcast SHA-256 against `release.json`, then atomically writes the already-signed bytes to ignored `site/public/appcast.xml` and a sanitized ignored receipt to `site/.staged/appcast.json`. It never generates, signs, contacts Keychain, or accepts a private key. `predeploy` reruns the site suite and refuses if staged bytes/receipt are missing or changed, preventing the documented ordinary `npm run deploy` path from silently removing or replacing a published feed.
- **Strict RED:** after adding the U6 route/staging smoke first, `npm run check` passed the unchanged static smoke and then exited `1` with `ERR_MODULE_NOT_FOUND` for absent `site/lib/appcast.mjs`. The production validator, route, and staging path were added only after that expected failure.
- **GREEN:** final `npm ci` installed 48 packages and audited 49 with 0 vulnerabilities. `npm run check` reports `static smoke: ok` and `appcast smoke: ok`; the appcast suite covers HTML fallback, malformed/unsigned feeds, content type, signatures, insecure/credential/API/draft-looking/non-exact URLs, title/version/build/name/length/minimum-OS mismatch, private-key text, deltas/channels/policies including namespace bypasses, exact Worker GET/HEAD/POST/query behavior, retained U5 historical items, complete prepared-directory staging, bad checksum rejection, and post-stage mutation rejection. The initially tried parser `5.2.5` exposed a critical audit advisory and was immediately replaced; final exact `5.10.1` has `npm audit --audit-level=high` exit `0`.
- A final real local Wrangler run proved `/` remains landing HTML, navigation-mode `/install` remains the honest shell stub, absent `/appcast.xml` and `/appcast.xml?release=10` return non-HTML `503` with exact failure headers, HEAD returns `503`, POST returns `405`, and trailing-slash/case/encoded variants never return feed XML. `npx wrangler deploy --dry-run` bundled the Worker and 14 unchanged static assets successfully without authenticating or deploying. The expected `npm run verify:staged-appcast` missing-artifact check exited `1` and confirmed deploy fails closed when no real feed is staged.
- `BESSIE_PROOF_URL=http://127.0.0.1:8797 npm run proof` passed at 1280 and 1440 widths, including copy/fallback, CTA configuration, advancing ages/watchdog, `/install`, horizontal overflow, browser errors, and reduced motion. Inspection of ignored `site/proof/landing-1280.png` found the full landing page intact with no clipping, missing assets, blank areas, or appcast error content. U6-scoped whitespace checks passed; no fake or real `site/public/appcast.xml` or `.staged` receipt exists in the final tree.
- Oracle first found that Static Assets could bypass the Worker, regex-only XML accepted malformed/policy variants, URL comparison normalized non-exact strings, and retained-item checks were narrower than U5. `run_worker_first`, canonical internal lookup, the bundled XML parser/tree checks, exact textual URLs, and U5-compatible retained notes fixed those findings. Follow-up Oracle found alternate-prefix and numeric-character-reference namespace bypasses; both were closed with focused regressions. Final Oracle verdict: **Approved**.
- **Deferred operator gates:** no real U5 prepared signed feed exists here, so staging and a real local/preview 200 response remain unclaimed. No preview deployment, `wrangler whoami`, Cloudflare account/hostname check, custom-domain route, DNS/MX/TXT change, production endpoint, GitHub visibility/release/draft/immutability/anonymous-asset check, enclosure byte/checksum download, Sparkle cryptographic verification, old-to-new update, credential use, package, install, relaunch, `mac-verify.sh`, commit, push, or PR occurred. Before preview, require exact account `6f03cccab9f19bc53ea7d259167a7024` and hostname `bessie-dev.jordanjstella.workers.dev`; before production, complete the separately approved public immutable release, anonymous byte verification, DNS/TLS/header/body proof, and installed-old-build update in KTD12 order with appcast publication last.

## 2026-08-09: Foreground-bound experimental TUI clicks and pane-label-first sidebar presentation

- Jordan superseded the initial Amp-only scope. The transient per-controller opt-in is now **Enable TUI Mouse Clicks (Experimental)** with no executable allowlist or TUI guessing. On the first fresh public Herdr `pane.process_info` response, Bessie binds to the foreground process-group fingerprint (PGID plus leader PID and argv-derived executable when represented) and continues only while fresh observations match. Errors, malformed/empty/mismatched observations, the 250 ms freshness limit, detach, reconnect/takeover, terminal-incarnation change, controller release, disable, and app/window/terminal focus loss revoke fail closed.
- Polling remains limited to an opted-in visible pane. Each request has a bounded 250 ms Bessie client deadline; timeout/error cancels the loader task and revokes, while a generation token prevents a late response from an invalidated request from affecting a re-enabled or replaced controller. The default transport remains the existing public Herdr socket `pane.process_info` request; no CLI polling or private protocol was added.
- TUI forwarding remains left-click-only with retained SGR 1006 cell coordinates. Bessie buffers the original down until mouse-up and emits one atomic press+release write only while authorization is fresh. If drag, Shift, disable, or authorization loss forces local selection, the original down is replayed to Ghostty before drag/up so selection receives a complete ordered gesture. Right click remains Bessie's context menu, wheel remains Herdr scrolling, and hover/drag/middle/right SGR stays disabled. Whether a target TUI requires 1016 pixel coordinates remains unproven and deferred.
- Joined each agent/sidebar projection to its connection-scoped pane projection by connection plus pane ID. `PaneProjection.presentationTitle` is primary; a distinct Herdr agent-session identity is rendered and announced as secondary. When it equals the primary or no matching pane projection exists, it is not duplicated. `AgentProjection.identity` remains unchanged globally, pane headers use the same pane presentation title, and rename still routes through public `pane.rename`.
- **Strict RED:** generalized tests first failed to compile because `TUIMouseClickGate`/`TUIMouseProcessInfo` did not exist. Review-fix tests then failed to compile on the absent deadline, async-loader, local-pointer observation, and distinct announcement seams. After the initial fallback implementation, `testTUIMouseFallbackReplaysBufferedDownBeforeDragAndUp` failed because the completed fallback incorrectly forced the next click local. Follow-up review tests then failed with three assertions proving an authorization loss at down emitted an unmatched full-capture SGR release and an interrupted fallback consumed the next gesture. Production changes followed each observed RED.
- **Mac GREEN:** in fresh scratch `/tmp/bessie-tui-mouse-red-1786258085`, `xcrun swift test --disable-keychain --disable-netrc --filter TerminalMouseRoutingTests` passed **14 tests**, `--filter 'SurfaceProjectionTests/testTUIMouse'` passed **6 tests**, and the three selected pane-label `HerdListTests` passed **3 tests**, all with zero failures. The app tests cover ordered local replay, authorization loss at down, interrupted/completed fallback reset, same-window first-responder revocation, owning-window notification isolation, bounded loader cancellation/late-response rejection, arbitrary-TUI binding, incarnation reset, atomic click output, Shift selection, right menu, wheel, and no drag/hover SGR.
- Task-scoped `git diff --check` passed. Focused Oracle follow-up and final scoped code review both approved the corrected tree with no actionable findings. Residual risk: the default synchronous Herdr socket operation runs in a detached task and may continue until the OS call returns after Bessie's 250 ms deadline, but its result cannot outlive the request generation or restore authorization. No full suite, `mac-verify.sh`, package, install, relaunch, commit, push, PR, publication, deployment, upstream Herdr/libghostty edit, or Herdr-owned runtime mutation was performed. Live process-info decoding against a current TUI pane, visual context-menu acceptance, and actual target-TUI 1006 compatibility remain manual checks.
- **Final P2 review pass:** process-info freshness now starts at request dispatch, so socket latency consumes the 250 ms authorization budget. That request alone opts into a Darwin `SO_RCVTIMEO` read deadline; the transport default remains unchanged for all other request and subscription users. A stalled-socket regression test verifies timed reads return and client connections close instead of accumulating blocked work. Herd rail and menu-bar Needs You/Working rows now use pane-derived primary titles and render/announce only distinct secondary agent identities, without duplicate fallback metadata.
- The final focused Mac test run was blocked before sync or execution because `jordan-macbook` timed out over SSH, and this Linux host has no Swift toolchain. Tests were written before production changes, but neither executable RED nor GREEN can be claimed for this final pass. Static verification passed: `./scripts/check-ui-copy.sh` and the 11-file scoped `git diff --check` both exited 0. Final Oracle review approved the scoped changes with no high-confidence blocker. Remaining risk is unverified macOS/Swift compilation and runtime behavior for Darwin socket timeout cleanup and the updated AppKit surfaces.
- **Absolute request-deadline follow-up:** replaced per-read inactivity timeout behavior with one monotonic request deadline shared by nonblocking Unix-socket connect, the complete write, and all partial response reads. Every readiness boundary recomputes the remaining budget, timeout closes idempotently, and default requests/subscriptions retain their original blocking behavior. API-level regressions cover repeated stalled responses and automatic close, slow-drip responses that cannot reset the budget, and a blocked write that proves request bytes reached the socket before expiry. A deterministic blocked Unix-domain connect fixture was not added because Darwin does not provide a stable local setup for it without relying on backlog timing.
- Tests were authored first but not executed: the Mac remains offline and Linux has no Swift toolchain, so no RED/GREEN runtime claim is made. Scoped static checks passed, and final Oracle review approved the deadline semantics, descriptor ownership, timeout fixture cleanup, and unchanged no-deadline behavior. Remaining risk is macOS compilation and live Darwin timing behavior until the focused `TransportTests` run on the Mac scratch.
- **Final Mac verification:** the first focused compilation failed at `Tests/BessieAppModelTests/MenuBarPresentationTests.swift:436` because the `PaneProjection` call supplied `title` before the initializer's required `agent` argument. The minimal argument-order-only correction was applied. The focused mouse/pane/status rerun then passed **141 tests, 1 skipped, 0 failures**. The complete Swift suite passed **688 tests, 6 skipped, 0 failures**, followed by Swift Testing at **21/21 passed**.
- A release `BessieApp` build succeeded with one pre-existing optional-`NSApp` warning. Verification packaging for identity `dev.bessie.app.verify` succeeded and the packaged verification app launched as PID **8802**. The production signed-install attempt failed closed because the noninteractive SSH context could not access the Developer ID key (`errSecInternalComponent`); screenshot capture also failed from the SSH display context. No production install, commit, push, publication, or deployment occurred.
