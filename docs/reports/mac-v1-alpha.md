...
The final verifier run launched that app against an isolated repository-local Herdr 0.7.5 server, proved automatic detached startup of the named `bessie` session despite inherited generic Herdr socket/session pollution, left `default` untouched, exercised the live terminal and topology paths, captured the native windows, and cleaned up only the processes it started.
...
- Swift tests: 56 executed, 0 failures.
- Debug build and production `BessieApp` build: passed without reported warnings.
- Packaging: `dist/Bessie.app` produced as version `0.1.0` build `3` with dark and light icon resources; plist validation and strict ad-hoc signature verification passed.
- Packaged executable: 13,026,608 bytes; SHA-256 `dbf7d2aeaeb5f566bf4c6628bca4d780074ea66da38d73c274ff7dce0f565f3e`.
- Candidate archive: `dist/Bessie-0.1.0-rc.3.zip`, 7,752,207 bytes; SHA-256 `da15555234e6e511be26e78876707bc2920d816583934047bef1e5fd49cb22e7`. Archive integrity, extraction, strict signature verification, default-icon metadata, and alternate-icon presence passed.
- `git diff --check`: passed.
- Verifier cleanup left no packaged test app, repository-local Herdr server, or test terminal controller running. The separately installed RC3 app and its user-owned named `bessie` session were then intentionally left running for hands-on acceptance.
...
- Runtime discovery, Herdr 0.7.5/protocol-17 compatibility, automatic detached startup of an isolated named `bessie` session, healthy-server reuse, ownership-safe cleanup, readiness polling, public socket ping, subscribe-before-snapshot bootstrap, event invalidation, snapshot reconciliation, bounded retry, isolated server restart, and reconnect. Generic inherited `HERDR_SOCKET_PATH` and `HERDR_SESSION` values cannot redirect Bessie to an unrelated session; only the explicit diagnostic `BESSIE_HERDR_SOCKET_PATH` override is honored. The verifier injected both generic values and still proved that `bessie` started while isolated `default` remained stopped.
...
- A native AppKit screenshot guard rejected light/warm regressions, missing cowprint, and undersized system thumbnails. The final RC3 Workspace capture measured rail luminance `11.09`, dark-pixel cowprint standard deviation `3.08`, and mean chroma `0.00`; Settings measured `13.40`, `4.07`, and `0.00` respectively. The Settings review found the Appearance control legible, aligned, and unclipped.
...
- Verified RC3 is installed at `/Applications/Bessie.app` on `jordan-macbook` as version `0.1.0` build `3`; strict signature verification passed after installation.
